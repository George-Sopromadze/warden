...
- **Optional two-model review.** Turn on a second opinion and Claude and Gemini each review the code independently, then a neutral judge that wrote neither review checks every concern against the actual code and decides. It catches issues a single model misses, and because the judge has no stake in either review, no model grades its own work. Off by default, and it falls back to single-model review if the second model is unavailable.
